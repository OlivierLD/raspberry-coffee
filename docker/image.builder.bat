...
then
  ...
  if [ "$a" == "Y" ]  || [ "$a" == "y" ]
  then
    echo Last generated one is $IMAGE_NAME:latest, its ID is $CONTAINER_ID"
  ...
  echo You can export a running container any time by running 'docker export --output export.tar [Container ID]'"
  echo Docker commands are documented at https://docs.docker.com/engine/reference/commandline/docker/"
fi
::
:eos
@endlocal
