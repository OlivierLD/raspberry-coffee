...
public class ImageDBUtils {
    ...
    public static List<LabelDefinition> getImageTagList(Connection conn) {
        List<LabelDefinition> tagList = new ArrayList<>();
        try {
            String labelStmt =
            ...
            Statement statement = conn.createStatement();
            ResultSet rs = statement.executeQuery(labelStmt);
            while (rs.next()) {
                String tag = rs.getString(1);
                int occ = rs.getInt(2);
                tagList.add(new LabelDefinition(tag, occ));
            }
            rs.close();
            statement.close();
        } catch (Exception e) {
