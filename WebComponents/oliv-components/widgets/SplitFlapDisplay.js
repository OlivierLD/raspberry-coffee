...
class SplitFlapDisplay extends HTMLElement {
	...
	static get observedAttributes() {
		return [
			...
			"justified"     // LEFT (default) or RIGHT
		];
	}
	...
	drawSplitFlap(textValue) {
		if (this._connected) {
			...
			let upperCaseValue = this._value.toUpperCase().split(''); // Char array
			...
			if (this._justified === 'RIGHT') {
				if (this._nb_char > upperCaseValue.length) {
					let nbPad = this._nb_char - upperCaseValue.length;
					for (let i=0; i<nbPad; i++) {
						this.drawOneFlap(context, ' ', i * oneWidth, 0, oneWidth, height, scale);
					}
					startAt = nbPad;
				}
			}
			...
			for (let i=0; i<upperCaseValue.length; i++) {
				this.drawOneFlap(context, upperCaseValue[i], (startAt + i) * oneWidth, 0, oneWidth, height, scale);
			}
			// 3 - Padding right?
			if (this._justified === 'LEFT') {
				if (this._nb_char > upperCaseValue.length) {
					let nbPad = this._nb_char - upperCaseValue.length;
					for (let i=0; i<nbPad; i++) {
						this.drawOneFlap(context, ' ', (i + upperCaseValue.length) * oneWidth, 0, oneWidth, height, scale);
					}
				}
			}
		}
