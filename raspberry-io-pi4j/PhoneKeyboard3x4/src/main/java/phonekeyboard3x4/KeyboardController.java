...
public class KeyboardController {
	...
	private Pin[] kpCol = new Pin[]{
			StringToGPIOPin.stringToGPIOPin(PinUtil.GPIOPin.GPIO_7.pin()),
			StringToGPIOPin.stringToGPIOPin(PinUtil.GPIOPin.GPIO_0.pin()),
			StringToGPIOPin.stringToGPIOPin(PinUtil.GPIOPin.GPIO_3.pin())
	};
	...
	public KeyboardController(boolean print) {
			...
			if (userProvidedCols == null || userProvidedRows == null) {
				...
				for (int i=0; i<userRows.length; i++) {
					...
					if (pin == null) {
						...
						kpRow[i] = StringToGPIOPin.stringToGPIOPin(pin.pin());
					}
				}
				for (int i=0; i<userCols.length; i++) {
		...
		if (print) {
			...
			System.out.println("        |  |  |  |  |  |  |");
			System.out.println("        |  |  |  |  |  |  Col [1,4,7,*] " + PinUtil.findByPin(kpCol[2].getName()).toString() + ", " + PinUtil.findByPin(kpCol[2].getName()).pinName());
			System.out.println("        |  |  |  |  |  Col [2,5,8,0] " + PinUtil.findByPin(kpCol[1].getName()).toString() + ", " + PinUtil.findByPin(kpCol[1].getName()).pinName());
			System.out.println("        |  |  |  |  Col [3,6,9,#] " + PinUtil.findByPin(kpCol[0].getName()).toString() + ", " + PinUtil.findByPin(kpCol[0].getName()).pinName());
			System.out.println("        |  |  |  Row [1,2,3] " + PinUtil.findByPin(kpRow[3].getName()).toString() + ", " + PinUtil.findByPin(kpRow[3].getName()).pinName());
			System.out.println("        |  |  Row [4,5,6] " + PinUtil.findByPin(kpRow[2].getName()).toString() + ", " + PinUtil.findByPin(kpRow[2].getName()).pinName());
			System.out.println("        |  Row [7,8,9] " + PinUtil.findByPin(kpRow[1].getName()).toString() + ", " + PinUtil.findByPin(kpRow[1].getName()).pinName());
			System.out.println("        Row [*,0,#] " + PinUtil.findByPin(kpRow[0].getName()).toString() + ", " + PinUtil.findByPin(kpRow[0].getName()).pinName());
			System.out.println();

			String[] map = new String[]{
					String.valueOf(PinUtil.findByPin(kpRow[0].getName()).pinNumber()) + ":" + "R1",
					String.valueOf(PinUtil.findByPin(kpRow[1].getName()).pinNumber()) + ":" + "R2",
					String.valueOf(PinUtil.findByPin(kpRow[2].getName()).pinNumber()) + ":" + "R3",
					String.valueOf(PinUtil.findByPin(kpRow[3].getName()).pinNumber()) + ":" + "R4",
