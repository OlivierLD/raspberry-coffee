...
process.title = 'node-pwm'; // Optional. You will see this name in eg. 'ps' or 'top' command
	...
	if (req.url.startsWith("/data/")) { // Static resource
		...
		if (req.method === "POST") {
			...
			req.on("end", function () {
				...
				if (req.headers["tyrus-ws-attempt"] !== "undefined" && req.headers["tyrus-ws-attempt"] === "Hand-Shake") { // List of the supported protocols
