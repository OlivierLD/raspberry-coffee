...
public class DecisionTablesStaticUtils {
    ...
    private final static String _AND_ = " AND | and ";
    ...
    private static DecisionContext setUpdateContext(Map<String, Object> decisionMap,
        ...
        if (where != null) {
            ...
            String[] andMembers = where.split(_AND_);

            for (String andMember : andMembers) {
                String[] whereSplit = andMember
                        .trim()
                        .split("=");
                context.addWhereColumnId(stripQuotes(whereSplit[0].trim()));
                context.addWhereColumnValue(stripQuotes(whereSplit[1].trim()));
            }
        }
