...
void SSD1306::drawString(int x, int y, String text) {
  for (int j = 0; j < text.length(); j++) {
    for (int i = 0; i < 8; i++) {
      unsigned char charColumn = pgm_read_byte(_font[text.charAt(j) - 0x20] + i);
      for (int pixel = 0; pixel < 8; pixel++) {
        if (bitRead(charColumn, pixel)) {
          if (_isFontScaling2x2) {
...
void SSD1306::setFontScale2x2(bool isFontScaling2x2) {
  _isFontScaling2x2 = isFontScaling2x2;
}
...
void SSD1306::sendCommand(unsigned char com) {
  // Wire.begin(_sda, _sdc);
  Wire.beginTransmission(_I2CAddress);      // begin transmitting
...
void SSD1306::nextFrameTick() {
  _frameTick++;
  if (_frameTick == _frameWaitTicks && _frameState == 0 || _frameTick == _frameTransitionTicks && _frameState == 1) {
    _frameState = (_frameState + 1) %  2;
    if (_frameState == FRAME_STATE_FIX) {
      _currentFrame = (_currentFrame + 1) % _frameCount;
    }
    _frameTick = 0;
  }
  drawIndicators(_frameCount, _currentFrame);

  switch (_frameState) {
    case 0:
      (*_frameCallbacks[_currentFrame])(0, 0);
      break;
    case 1:
      (*_frameCallbacks[_currentFrame])(-128 * _frameTick / _frameTransitionTicks, 0);
      (*_frameCallbacks[(_currentFrame + 1) % _frameCount])(-128 * _frameTick / _frameTransitionTicks + 128, 0);
      break;
  }
}
