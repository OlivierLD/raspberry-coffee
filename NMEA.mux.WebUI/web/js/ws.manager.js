...
function setValues(doc) {
    try {
        ...
        /*
        let displayWT = (json.displayWT !== undefined ? json.displayWT : true);

        events.publish('show-hide-wt', displayWT);

        try {
            let latitude = parseFloat(json.lat);
            let longitude = parseFloat(json.lng);
            events.publish('pos', {
                'lat': latitude,
                'lng': longitude
            });
