...
//  console.log('Color scheme has changed');
	...
	function rotateBothWays(lat, lng, rotationAroundY, rotationAroundX, addToLng) {
		...
		var x = Math.cos(Math.toRadians(lat)) * Math.sin(Math.toRadians(lng + addToLng));
		var y = Math.sin(Math.toRadians(lat));
		var z = Math.cos(Math.toRadians(lat)) * Math.cos(Math.toRadians(lng + addToLng));

		var alfa = Math.toRadians(rotationAroundY); // in plan (x, z), y unchanged.
		var beta = Math.toRadians(rotationAroundX); // in plan (y, z), x unchanged. Tilt.
		/*
		 * Note:
