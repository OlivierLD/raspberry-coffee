        ...
        for (var i=0; i<json.length; i++) {
            ...
            switch (type) {
                ...
                case 'serial':
                    ...
                    break;
                case 'tcp':
                    html += ("<tr><td><b>tcp</b></td><td>Port " + json[i].port + "</td><td><button onclick='removeForwarder(" + JSON.stringify(json[i]) + ");'>remove</button></td><td>" + json[i].nbClients + " Client(s)</td></tr>");
                    break;
                case 'ws':
