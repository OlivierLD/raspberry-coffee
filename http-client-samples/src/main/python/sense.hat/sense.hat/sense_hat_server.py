...
def read_sense_hat():
    ...
    while keep_looping:
        try:
            try:
                hum = sense.humidity
                core.update_cache('hum', hum)
            except AttributeError as ae:
                print("AttributeError : {}".format(ae))
            try:
                temp = sense.temp
                core.update_cache('air', temp)
            except AttributeError as ae:
                print("AttributeError : {}".format(ae))
            try:
                press = sense.pressure
                core.update_cache('pmsl', press)
            except AttributeError as ae:
                print("AttributeError : {}".format(ae))

            try:
                orientation_deg = sense.get_orientation_degrees()   # get_orientation_radians()
...
try:
    print("Starting!")
    x = threading.Thread(target=read_sense_hat)
    x.start()
except OSError as ose:
    print(ose)
    sys.exit(1)  # Bam!

PATH_PREFIX = "/sense-hat"
